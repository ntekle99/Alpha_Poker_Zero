"""Evaluation script for PPO-trained poker models."""
...
def evaluate_ppo_model(
            ...
            for hand_num in range(hands_per_game):
                ...
                if game.is_terminal(state):
                    ...
                    if state.winner == 1:
                        ...
                        p2_chips = state.player2_stack + (state.pot - state.pot // 2)  # Handle odd pots
    ...
    print(f"\nGame win rates:")
    if vs_random:
        print(f"  PPO Model: {model_game_wins} ({100 * model_game_wins / num_games:.1f}%)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate PPO-trained poker models")
    parser.add_argument("--model", type=str, required=True,
                        help="Path to PPO model to evaluate")
